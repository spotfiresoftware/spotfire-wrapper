...
class PageState {
  index: number; pageTitle: string;
}
class DataTable {
  dataTableName: string;
}
class DataColumn {
  dataColumnName: string; dataTableName: string; dataType: string; values: any;
}
...
export class SpotfireDocument {
  ...
  constructor(app, ids, page, custo) {
    ...
    for (const id of ids) {
      ...
      this.docs[id] = {};
      // TODO: Assign to a tuple of docs
      // this._doc = app.openDocument(id, page, custo);
      this.docs[id] = app.openDocument(id, page, custo);
    }
    app.onOpened$().subscribe(doc => {
      doConsole(`Document.onOpened$: page is now opened:`, doc);
      console.log('DOC Open:', doc);
      this._doc = doc;
...
const doCall = <T>(obj, m: string, ...a: any[]): Observable<T> => {
  ...
  return new Observable<T>(observer => {
    ...
    try {
      ...
      const p = (g: T) => {
        observer.next(g); observer.complete();
      };
